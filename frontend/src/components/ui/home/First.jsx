...
function First() {
  return (
    <section id="first" className="relative w-full h-auto  bg-gradient-to-l from-[rgba(230,230,250,0.2)] to-RuqyaLightPurple bg-white overflow-x-clip">
      
      {/* Overlay to prevent background from affecting content */}
      <div className="absolute inset-0 bg-white opacity-50"></div>
      
      {/* SVG Background - Hidden on mobile, visible on large screens */}
      <div className="hidden lg:block absolute right-[-3%] -top-10 pointer-events-none animate-fade-in w-full max-w-[300px] xl:max-w-[600px] 2xl:max-w-[800px]" style={{ animationDelay: "0.2s" }}>
        <svg className=" md:max-w-[550px]  md:max-h-[550px] lg:max-h-[600px] lg:max-w-[600px]  ml-auto" xmlns="http://www.w3.org/2000/svg" viewBox="8.19 7.39 16.4 18">
      ...
      <div className="relative h-full flex items-center justify-center mx-[8%]">
        <div className="mx-auto px-3 w-full">
          <div className="grid grid-cols-1 lg:grid-cols-2 items-center gap-8 relative z-10">
            {/* Text Content */}
            ...
            {/* Image Container */}
            <div className="flex items-center justify-end mb-3 mt-10 md:mt-0 lg:mb-0 order-1 lg:order-2 animate-fade-in" style={{ animationDelay: "1s" }}>
              <div className="relative w-full md:w-[90%]">
                <img src="https://miro.medium.com/v2/resize:fit:1200/1*dYuIVOkIcDKIarat5ynsIw.jpeg" 
                    alt="men-meditation" 
                    className="w-full h-auto object-cover md:my-5 object-center scale-x-[-1] lg:rounded-[3.2rem] rounded-xl" />
              </div>
            </div>
          </div>
        </div>
