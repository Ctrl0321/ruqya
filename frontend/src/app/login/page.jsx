  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response && response.role === "user") {
        console.log("Test 1",response.role,response.isEmailVerified)

        if (!response.isEmailVerified){
          const newOtp = generateOtp();
          console.log("Test2",newOtp)
  ...
  return (
    <main className="min-h-screen flex items-center justify-center relative text-sm md:text-lg">
      ...
      <div className="w-full flex items-center justify-center relative z-10">
        ...
        {/* Center side - Form */}
        <div className="w-fit  mx-auto  md:mt-10 animate-fade-in " style={{ animationDelay: `500ms` }}>
          <div className="bg-white rounded-3xl p-8 shadow-xl m-5">
            ...
            { !otpSent ?
                (            <form onSubmit={handleSubmit} className="space-y-8">
                      <BorderInput label="Email Address" type="email" name="email" placeholder="Enter your Email Address here" className="text-sm !sm:w-[400px] md:w-[350px]" value={email} onChange={(e) => setEmail(e.target.value)} />
                ...
                ( <form onSubmit={handleOtpVerify} className="space-y-8">
                  ...
                  <Button type="submit" bg={true} text={isLoading ? "Verifying..." : "Verify"} color={"RuqyaGreen"} disabled={isLoading} className="w-full bg-teal-600 hover:bg-teal-700 text-white rounded-full py-3 mt-5" />
                </form>)
            }
