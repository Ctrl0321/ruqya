...
export const getMyBookings = async () => {
  const bookings = await api.get(`ruqya-api/meeting/get-meetings/user/?timeZone=${userTimeZone}`).data;
  const detailedBookings = await Promise.all(
    bookings.map(async (booking) => {
      const rakiData = await api.get(`ruqya-api/raki/${booking.rakiId}`).data;
      return { ...booking, rakiData };
    })
  );
