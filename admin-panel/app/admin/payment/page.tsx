...
import { Search, Calendar, DollarSign, Clock, Filter, X } from 'lucide-react';
...
const PaymentManagementPage: React.FC<PaymentManagementPageProps> = ({
    ...
    const meetings: Meeting[] = [
        {
            ...
            isPaid: false,
            requestedAt: "2025-02-03 23:00:00"
        },
        {
            _id: "67a246ef6565937eae03bad0",
            meetingId: "2000",
        ...
        },
        {
            ...
            notificationSend: true,
            isPaid: false,
            requestedAt: "2025-02-01 09:00:00"
        }
    ];
    ...
    const filteredMeetings = useMemo(() => {
        return meetings.filter(meeting => {
            ...
            const monthMatch = !selectedMonth ||
                meetingDate.toLocaleString('default', { month: 'long' }).toLowerCase() === selectedMonth.toLowerCase();
            const yearMatch = !selectedYear ||
                meetingDate.getFullYear().toString() === selectedYear;
            const statusMatch = !paymentStatus ||
                (paymentStatus === 'paid' && meeting.isPaid) ||
                (paymentStatus === 'unpaid' && !meeting.isPaid) ||
                (paymentStatus === 'requested' && meeting.requestedAt && !meeting.isPaid);
            const searchMatch = !searchTerm ||
                meeting.userId.toLowerCase().includes(searchTerm.toLowerCase()) ||
                meeting.topic.toLowerCase().includes(searchTerm.toLowerCase());
            const userMatch = !isAdmin ? meeting.userId === currentUserId : true;

            return monthMatch && yearMatch && statusMatch && searchMatch && userMatch;
        });
    }, [meetings, selectedMonth, selectedYear, paymentStatus, searchTerm, isAdmin, currentUserId]);
    ...
    const stats = useMemo(() => {
        ...
        const requested = filteredMeetings.filter(m => m.requestedAt && !m.isPaid).length;
            ...
            <div className="max-w-7xl mx-auto">
                        ...
                        >
                            <div className="p-4">
                                <div className="flex justify-between items-start mb-3">
                                    <h3 className="font-medium text-lg text-foreground">{meeting.topic}</h3>
                                    {meeting.isPaid ? (
                                    ...
                                    <div className="mt-4">
                                        {isAdmin ? (
                                            <button
                                                ...
                                                onClick={() => console.log('Mark as paid:', meeting._id)}
                                            >
                                                Mark as Paid
                                            </button>
                                        ) : (
                                            canRequestPayment(meeting) ? (
                                                <button
                                                    className="w-full px-4 py-2 bg-primary-500 text-primary-25 rounded-md hover:bg-primary-600 transition"
                                                    onClick={() => console.log('Request payment:', meeting._id)}
                                                >
                                                    Request Payment
                                                </button>
                                            ) : (
                ...
                {filteredMeetings.length === 0 && (
                    <div className="text-center py-12">
                        <p className="text-lg text-muted-foreground">No meetings found matching your filters</p>
                    </div>
                )}
            </div>
        </div>
